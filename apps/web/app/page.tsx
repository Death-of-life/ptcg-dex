...
const TopBar = memo(function TopBar({ lang, topbarHidden, onSwitchLang }: TopBarProps) {
  return (
...
export default function HomePage() {
  ...
  const [langTotals, setLangTotals] = useState<Record<Lang, number>>({
    en: 0,
    ja: 0,
    "zh-tw": 0
  });
  ...
  useEffect(() => {
    ...
    fetchCards(lang, { page: 1, pageSize: 1, sortBy: "updatedAt", sortOrder: "desc" }, { signal: controller.signal })
      .then((res) => {
        if (cancelled) return;
        setLangTotals((prev) => ({ ...prev, [lang]: res.total ?? 0 }));
      })
      .catch(() => {
        if (cancelled) return;
        setLangTotals((prev) => ({ ...prev, [lang]: prev[lang] || 0 }));
      });
  ...
  const openRandomCard = useCallback(async () => {
    const count = langTotals[lang] || total;
  ...
  }, [lang, langTotals, total, openCardDetail]);
  ...
  return (
    <main className="page-shell">
      <TopBar lang={lang} topbarHidden={topbarHidden} onSwitchLang={setLang} />
      ...
      <section className="content-area">
        <aside className="desktop-filters">
          <FilterPanel current={current} filters={filterOptions[lang]} onChange={updateFilter} onReset={resetFilters} />
        </aside>
